    ...
    @if (store.showCzentrix()) {
      @if (!hasFooter()) {
        <button
          ...
          type="button"
          zSize="sm"
          class="fixed bottom-12 right-4 z-[70] gap-1 shadow-lg"
          (click)="store.toggleCti()"
        >
          <ng-icon name="lucidePhone" size="14" aria-hidden="true" />
          {{ store.czentrixLabel }}
        </button>
      }
      ...
      @if (store.ctiUrl(); as src) {
        ...
        -->
        <div
          class="fixed bottom-28 right-4 z-[70] flex w-[178px] flex-col overflow-hidden rounded-md border border-border bg-card shadow-lg"
...
export class CtiPanelComponent {
  ...
  readonly hasFooter = computed(() => FOOTER_ROUTE_PREFIXES.some((prefix) => this.store.url().startsWith(prefix)));
