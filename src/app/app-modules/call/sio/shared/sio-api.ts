...
/**
 * Shared HTTP contract for the SIO service tabs (blood-on-call, epidemic, food
 * safety, grievance, organ donation, schemes, IMR/MMR, bal vivah). Every SIO
 * endpoint returns the standard AMRIT envelope and each service normalises
 * failures to a {@link SioError}, so the envelope, error type and the
 * read/normalise helpers live here rather than being copied per service.
 */
...
/** A normalised SIO-API error a component can display. */
export interface SioError {
  status: number;
  errorMessage: string;
}
 ...
 * Throws a {@link SioError} so the pipe's `catchError` can forward it.
 */
export function readSioData<T>(res: ApiResponse<T>): T | undefined {
...
/** Normalise any thrown value (timeout, envelope, HTTP error) to a {@link SioError}. */
export function toSioError(err: unknown): SioError {
