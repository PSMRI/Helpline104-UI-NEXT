...
describe('PrescriptionComponent', () => {
  ...
  describe('Resend Prescription SMS (legacy case-sheet-recentPrescription-modal)', () => {
    const history = [
      {
        prescriptionID: 5,
        diagnosisProvided: 'Fever',
        createdDate: new Date().toISOString(),
        prescribedDrugs: [{ prescribedDrugID: 501, drugName: 'Paracetamol' }],
      },
    ];

    it('cannot send until a drug line is selected', () => {
      const fixture = render(history);
      expect(fixture.componentInstance.canSendResend()).toBeFalse();

      fixture.componentInstance.toggleResendDrug(501);
      expect(fixture.componentInstance.canSendResend()).toBeTrue();
    });
    ...
    it('rejects an alternate number that is not exactly 10 digits', () => {
      ...
      fixture.componentInstance.resendAltNumber.set('12345');
      expect(fixture.componentInstance.altNumberValid()).toBeFalse();
      expect(fixture.componentInstance.canSendResend()).toBeFalse();

      fixture.componentInstance.resendAltNumber.set('9876543210');
      expect(fixture.componentInstance.altNumberValid()).toBeTrue();
      expect(fixture.componentInstance.canSendResend()).toBeTrue();
    });
    ...
    it('sends one SMS request per selected drug via the Prescription SMS template', () => {
      ...
      fixture.componentInstance.sendResendSms();
      ...
      expect(fixture.componentInstance.selectedResendDrugIds().size).toBe(0);
    });
    ...
    it('no-ops when the service has no Prescription SMS type configured', () => {
      ...
      expect(fixture.componentInstance.sendingSms()).toBeFalse();
