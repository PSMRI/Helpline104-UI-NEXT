...
export class CampaignService {
  ...
  switchToInbound(agentId: number): Observable<unknown> {
    return this.http.post(`${this.config.getOpenCommonBaseURL()}${SWITCH_TO_INBOUND_PATH}`, {
      agent_id: agentId,
    });
  }
  ...
  switchToOutbound(agentId: number): Observable<unknown> {
    return this.http.post(`${this.config.getOpenCommonBaseURL()}${SWITCH_TO_OUTBOUND_PATH}`, {
      agent_id: agentId,
    });
  }
}
